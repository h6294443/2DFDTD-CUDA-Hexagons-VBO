...
void update_all_fields_hex_CUDA()
{
	...
	//Source_Update_Kernel << <BLK, THD >> >(dev_ez, dev_ez_float, src_pos_x, src_pos_y, 0, g->time, factor, 2, N_lambda, Sc, 0, maxTime);
	...
	/*cudaMemcpy(h1_check, dev_h1, M*N*sizeof(double), cudaMemcpyDeviceToHost);
	cudaMemcpy(h2_check, dev_h2, M*N*sizeof(double), cudaMemcpyDeviceToHost);
	cudaMemcpy(h3_check, dev_h3, M*N*sizeof(double), cudaMemcpyDeviceToHost);
	cudaMemcpy(ez_check_double, dev_ez, M*N*sizeof(double), cudaMemcpyDeviceToHost);
	
	printf("\nLoop #%i (Magnetic Fields)\n", g->time);
	for (int k = 0; k < M*N; k++) {
		if (h1_check[k] != 0)
			printf("H1[%i] = %g\n", k, h1_check[k]);
		if (h2_check[k] != 0)
	...
	Source_Update_Kernel << <BLK, THD >> >(dev_ez, dev_ez_float, src_pos_x, src_pos_y, 0, g->time, factor, 128, N_lambda, Sc, 0, maxTime);
